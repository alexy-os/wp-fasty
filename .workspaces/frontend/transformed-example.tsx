const posts = [{
  ...
  date: {
    ...
    display: 'January 1, 2021'
  },
  excerpt: 'This is the excerpt for Post 1'
}];
{
  posts.map((post: any) => <article key={post.id} data-slot="article" className="article">
      <header data-slot="article-header" className="article-header">
        ...
        <div data-slot="article-meta" className="article-meta">
          {post.date && <time dateTime={post.date.formatted} data-slot="article-time" className="article-time" datetime="">{post.date.display}</time>}
        </div>
      </header>
